    ...
    public void processCommand(ICommandSender sender, String[] args) {
        if (args.length == 1) {
            try {
                ...
                {
                    EntityPlayer player = sender.getEntityWorld().getClosestPlayer(
                        sender.getPlayerCoordinates().posX, sender.getPlayerCoordinates().posY,sender.getPlayerCoordinates().posZ, 1
                        );
                    player.openGui(CigaretteMod.instance, 420, sender.getEntityWorld(), (int) player.posX, (int) player.posY, (int) player.posZ);
                }
